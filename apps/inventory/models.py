...
class StockType(BaseModel):
    ...
    def total_ctn_quantity(self) -> int:        
        # Sum IN transactions
        in_total = self.transactions.filter(
            transaction_type="IN"
        ).aggregate(total=Sum('ctn_quantity'))['total'] or 0
        
        # Subtract OUT transactions
        out_total = self.transactions.filter(
    ...
    def total_piece_quantity(self) -> int:
        ...
        in_total = self.transactions.filter(
            transaction_type="IN"
        ).aggregate(total=Sum('piece_quantity'))['total'] or 0
...
class StockTransaction(BaseModel):
    ...
    price = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        related_name="stock_transactions",
        help_text="Price reference for this transaction"
    )
    ...
    ctn_quantity = models.PositiveIntegerField(
        default=0,
        help_text="Carton quantity for this transaction"
    )
    piece_quantity = models.PositiveIntegerField(
        default=0,
        help_text="Piece quantity for this transaction"
    )
    have_transfer = models.BooleanField(
        default=False,
        help_text="Whether this transaction involves a transfer"
    )
